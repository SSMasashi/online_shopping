...
ソースコードは `src/buy_sl.py` の1ファイルにまとまっており、Streamlit Cloud上で動かすことを想定しています。

## 主な機能

- **商品ごとのAmazon・楽天の価格入力**
  - Amazon側は価格・還元率（%）に加えて「らくベビ割（10%OFF）」のチェックに対応
  - 楽天側は価格・還元率（%）に加えて、商品ページのURLを貼り付けて「取得」ボタンを押すと、楽天市場商品検索APIから価格とポイント還元率を自動取得
- **最安の購入先の自動振り分け**
  - 商品数分のAmazon/楽天の組み合わせを総当たりで評価し、実質負担額が最も安くなる組み合わせを算出
  - 「最適な振り分け」「すべてAmazon」「すべて楽天」の3パターンの実質負担額を比較表示
  - 商品ごとの購入先・価格・還元倍率・還元ポイント・実質負担額の一覧と、Amazon/楽天ごとの詳細な計算内訳を表示
- **楽天お買い物マラソン（買いまわり）の考慮**
  - 買いまわり対象の最大ショップ数、対象となる最低金額（税込）、特典ポイントの上限、SPU倍率を画面上で設定可能
- **入力内容のGoogle Sheetsへの保存・読み込み・削除**
  - 商品リストと設定値を「保存名」を付けて複数パターン保存できる
  - 同名で保存しようとした場合は上書き確認、削除時も確認ダイアログを表示
- 商品は最大15件まで登録可能
...
1. Google Cloudでプロジェクトを作成し、Google Sheets APIとGoogle Drive APIを有効化する（アプリが要求するスコープは `spreadsheets` と `drive`）
